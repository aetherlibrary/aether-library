...
圖書館本身並不只是裝飾。場景、角色與閱覽室，都是這個產品呈現其成果的一部分。
目前，你的旅程將從 **Classic Library** 開始。

Aether Library 不只是一個傳統的對話式 AI 工具。 我們希望你能透過探索場景、與角色互動的方式和 AI 一起思考與討論。
未來，更多互動與遊戲性功能將逐步加入，讓這座圖書館從一個 AI 工作空間，慢慢成為一個真正生動、可以探索的世界。
...
Obsidian vault；內建的 Vault 仍然是主要系統，Obsidian的安裝 並非必要條件。

**檔案庫（Archives）** — 每一場儲存的討論都會記錄在本機，讓你之後可以重新開啟並繼續討論。
即使關閉或重新啟動 Aether Library，先前的討論也不會消失。你可以隨時從檔案庫延續過去的討論，
Aether Library 會將先前的對話內容一併提供給你選擇的 AI，讓討論能延續原有脈絡，並形成一個可隨時回顧的完整討論串。
...
**兩種語言** — 介面提供英文與繁體中文，而大智者回覆時所使用的語言，可以與介面語言分開設定。
預設情況下，AI 會依照你提問時使用的語言回答。
...
Aether Library 採用 local-first 的設計。你的知識庫、討論紀錄與設定都保存在自己的電腦上，
Aether Library 不需要帳號，也沒有自己的雲端伺服器來儲存或轉送你的 AI 對話。

**保存在本機的資料**

- 你的 API 金鑰，存放在本機的 `.env.local` 檔案中
- Vault 以純 Markdown 檔案儲存在你選擇的資料夾
- 已儲存的討論與 Archives 保存在本機
- Aether Library 的應用程式設定保存在本機

**使用 AI 時會傳送的內容**

- 當你開始一場討論時，你的問題以及該次請求所需要的附件、對話脈絡或 Vault 內容，會直接傳送給你所選擇的 AI 供應商。
- 例如，當你選擇 OpenAI、Anthropic 或其他雲端 AI 服務時，相關內容會依照該供應商的服務條款與隱私權政策處理，API 用量與費用也會計入你自己的供應商帳號。

Aether Library 本身不會收取或估算這些 API 費用。實際用量與費用請以各 AI 供應商的後台紀錄為準。
